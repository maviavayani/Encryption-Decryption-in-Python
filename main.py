...
def code_word(user1, key):
    ...
    for i in b:
        if (len(i)<3):
            ...
            code=i[1:] + i[0]
            code1 = (''.join(random.choice(string.ascii_lowercase) for _ in range(key))
            + code + ''.join(random.choice(string.ascii_lowercase) for _ in range(key)))
            lst.append(code1)
    a = ' '.join(lst)
    return a
    ...
    for i in b:
        if(len(i)<3):
            ...
            try:
                ...
                print("Your encryption key is greater than key")
            lst.append(code1)
    code=' '.join(lst)
    return code
while True:
    ...
    if(u.lower()=="code"):
        ...
        try:
            user2=int(input("Enter your key is (3 or 4 or 6): "))
        except Exception as e:
            pass
        try:
            print(code_word(user1,user2))
        except Exception as e:
            print("please enetr only digit for encryption:")
    elif(u.lower()=="decode"):
        user1=input("Enter your message:")
        try:
